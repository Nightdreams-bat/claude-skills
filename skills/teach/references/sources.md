...
```bash
python "~/.claude\skills\teach\scripts\ingest.py" "<url>" "StudyVault\<Area>\_sources\<slug>.md"
```

Python lives at `python` if bare `python` resolves to the Windows Store stub.
...
```bash
python "~/.claude\skills\teach\scripts\ingest.py" "<file.pdf>" "StudyVault\<Area>\_sources\<slug>.md"
```
