...
1. Check `StudyVault\` exists; create it and `concepts/` if not.
...
`/tutor` discovers the vault by globbing `**/StudyVault/` from the **current working directory**. So it only finds this one when run from inside `<your vault root>` (or a parent of it). When you mention `/tutor` at the end of a session, mention that too.
